...
module Make(O : OBJ)(EVP : EVALPATH with type valu = O.t) = struct
    ...
    let exn_printer ppf exn =
      fprintf ppf "%s" (Printexc.to_string exn)

    let out_exn path exn =
      Oval_printer (fun ppf -> exn_printer ppf exn)

    let install_printer path ty fn =
      let print_val ppf obj =
        try fn ppf obj with exn -> exn_printer ppf exn in
    ...
    and find_printer depth env ty =
      ...
      | (name, Generic (path, fn)) :: remainder ->
          ...
          | Tconstr (p, args, _) when Path.same p path ->
              ...
              with exn -> (fun obj -> out_exn path exn) end
