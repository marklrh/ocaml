...
let pp_ident_name ppf ident =
  let name =
    match ident.Ident.name with
    | "*match*" -> "s_match_s" (* normalize bytecomp/matching.ml L3215 *)
    | s when String.length s > 0 -> begin
        let fc = String.get s 0 in
        if fc = Char.lowercase_ascii fc then
          s
        else
          Format.sprintf "%s_%s" "module" (String.lowercase_ascii s)
      end
    | s -> s in
  Format.fprintf ppf "%s" name

let ident_print ppf (i:Ident.t) =
  match i.Ident.stamp with
  | 0 -> fprintf ppf "%s!" i.Ident.name
  | -1 -> fprintf ppf "%s#" i.Ident.name
  | n -> fprintf ppf "%a_%i%s" pp_ident_name i n (if Ident.global i then "g" else "")
...
let primitive ppf = function
  ...
  | Pgetglobal id -> fprintf ppf "global %a" ident_print id
  | Psetglobal id -> fprintf ppf "setglobal %a@." ident_print id
...
let rec lam ppf = function
  ...
  | Lfunction{kind; params; body; attr} ->
      let pr_params ppf params =
        match kind with
        | Curried ->
            List.iter (fun param -> fprintf ppf "@ %a" ident_print param) params
        | Tupled ->
            fprintf ppf " (";
      ...
      let rec letbody = function
        | Llet(str, k, id, arg, body) ->
            fprintf ppf "@ @[<2>%a =%s%s@ %a@]"
              ident_print id (kind str) (value_kind k) lam arg;
            letbody body
        | expr -> expr in
      ...
      let bindings ppf id_arg_list =
        ...
        List.iter
          (fun (id, l) ->
            ...
            fprintf ppf "@[<2>%a@ %a@]" ident_print id lam l)
          id_arg_list in
      fprintf ppf
  ...
  | Lstaticcatch(lbody, (i, vars), lhandler) ->
      fprintf ppf "@[<2>(catch@ %a@;<1 -1>with (%d%a)@ %a)@]"
        ...
        (fun ppf vars -> match vars with
          ...
          | _ ->
              List.iter
                (fun x -> fprintf ppf " %a" ident_print x)
                vars)
        vars
        lam lhandler
  | Ltrywith(lbody, param, lhandler) ->
      fprintf ppf "@[<2>(try@ %a@;<1 -1>with %a@ %a)@]"
        lam lbody ident_print param lam lhandler
  ...
  | Lfor(param, lo, hi, dir, body) ->
      fprintf ppf "@[<2>(for %a@ %a@ %s@ %a@ %a)@]"
       ident_print param lam lo
       (match dir with Upto -> "to" | Downto -> "downto")
       lam hi lam body
  | Lassign(id, expr) ->
      fprintf ppf "@[<2>(assign@ %a@ %a)@]" ident_print id lam expr
  ...
  | Levent(expr, ev) ->
      let kind =
       ...
       | Lev_module_definition ident ->
         Format.asprintf "module-defn(%a)" ident_print ident
      in
      fprintf ppf "@[<2>(%s %s(%i)%s:%i-%i@ %a)@]" kind
              ...
              lam expr
  | Lifused(id, expr) ->
      fprintf ppf "@[<2>(ifused@ %a@ %a)@]" ident_print id lam expr
...
(* this function is too long. just remove it *)
let name_of_primitive = fun _ -> assert false

module Strato_lambda = struct
  ...
  let todo reason = failwith (Printf.sprintf "TODO: %s" reason)
  ...
  let rec struct_const ppf = function
    ...
    | Const_base(Const_int64 n) -> fprintf ppf "%LiL" n
    | Const_base(Const_nativeint n) -> fprintf ppf "%nin" n
    | Const_pointer n -> fprintf ppf "%i" n (* rli: removed `a`. why `a` is here? *)
    | Const_block(tag, []) ->
        fprintf ppf "[%i]" tag
    | Const_block(_tag, consts) ->
      let pp_consts ppf cs =
        list ~sep:"," struct_const ppf cs in
      fprintf ppf "@[<1>Seq(%a)"  pp_consts consts
    | Const_float_array [] ->
        fprintf ppf "[| |]"
    | Const_float_array (f1 :: fl) ->
        let floats ppf fl =
  ...
  let _block_shape ppf shape = match shape with
    | None | Some [] -> ()
    | Some l when List.for_all ((=) Pgenval) l -> ()
    | Some [elt] ->
        Format.fprintf ppf " (%s)" (field_kind elt)
  ...
  let _pp_any_typ p ppf t = pp ppf "%a: Any" p t

  let pp_curry_arg ppf t = pp ppf "{%a: Any => " ident_print t
  let pp_tuple_arg ppf t = pp ppf "%a: Any" ident_print t
  ...
  let rec lam ppf = function
    ...
    | Lapply ap ->
        let arg fmt larg =
          pp fmt "(%a)" lam larg in
        let lams fmt largs = list ~sep:"@," arg fmt largs in
        (* TODO: body *)
        fprintf ppf "@[<2>(%a%a)@]" lam ap.ap_func lams ap.ap_args
    | Lfunction{kind; params; body} -> begin
        match kind with
        | Curried ->
            let ps fmt params = list ~sep:"@ " pp_curry_arg fmt params in
            pp ppf "@[<2>%a%a%s@]" ps params lam body
              (String.concat "" (List.map (fun _ -> "}") params))
        | Tupled ->
            let ps fmt params = list ~sep:", " pp_tuple_arg fmt params in
            pp ppf "@[<2>{(%a) => %a}@]" ps params lam body
      end
    | Llet(_str, _k, id, arg, body) ->
        ...
        let rec letbody = function
          | Llet(_str, _k, id, arg, body) ->
              fprintf ppf "val %a =@ %a@.@."
                ident_print id lam arg;
              letbody body
          | expr -> expr in
        fprintf ppf "val %a =@ @[<2>%a@]@.@."
          ident_print id lam arg;
        let expr = letbody body in
        fprintf ppf "%a@." lam expr
    | Lletrec(_id_arg_list, _body) -> failwith "Unsupported struct: letrec"
    | Lprim(prim, largs, _) -> begin
        pp_primitive ppf prim largs
      end
    | Lswitch(_larg, _sw, _loc) -> todo "switch"
        (*
        let switch ppf sw =
          let spc = ref false in
          List.iter
        ...
        fprintf ppf
         ...
         (match sw.sw_failaction with None -> "switch*" | _ -> "switch")
         lam larg switch sw *)
    | Lstringswitch(_arg, _cases, _default, _) -> todo "stringswitch"
    (*
        let switch ppf cases =
          let spc = ref false in
          List.iter
    ...
    | Ltrywith(lbody, param, lhandler) ->
        fprintf ppf "@[<2>(try@ %a@;<1 -1>with %a@ %a)@]"
          lam lbody ident_print param lam lhandler *)
    | Lifthenelse(lcond, lif, lelse) ->
        fprintf ppf "@[<2>if@ (%a)@ @ {%a}@ else@ @ {%a}@]"
          lam lcond lam lif lam lelse
    | Lsequence(l1, l2) -> (* this is  `x; y` - leave to TODO *)
        fprintf ppf "@[<2>(seq@ %a@ %a)@]" lam l1 sequence l2
    | Lwhile(_lcond, _lbody) -> todo "no while-loop"
        (* fprintf ppf "@[<2>(while@ %a@ %a)@]" lam lcond lam lbody *)
    | Lfor(_param, _lo, _hi, _dir, _body) -> todo "no for-loop"
                                          (*
        fprintf ppf "@[<2>(for %a@ %a@ %s@ %a@ %a)@]"
         ident_print param lam lo
         (match dir with Upto -> "to" | Downto -> "downto")
         lam hi lam body *)
    | Lassign(_id, _expr) -> (* no assignment allowed. could create rec *)
        failwith "assignment not allowed"
        (* fprintf ppf "@[<2>(assign@ %a@ %a)@]" ident_print id lam expr *)
    | Lsend (_k, _met, _obj, _largs, _) -> todo "class/object"
    | Levent(expr, _ev) ->
        fprintf ppf "@[<2>%a@]" lam expr
        (*
        let kind =
         match ev.lev_kind with
         | Lev_before -> "before"
...
*)
  ...
  and pp_lams ppf lambdas =
    list ~sep:"@ " lam ppf lambdas

  and pp_binary_prim_op ppf op args =
    match args with
    | [e1; e2] -> pp ppf "%a@ %s@ %a" lam e1 op lam e2
    | _ -> failwith "not binary"

  and pp_primitive ppf lprim args =
    match lprim with
    | Pidentity -> fprintf ppf "id"
    | Pbytes_to_string -> fprintf ppf "bytes_to_string"
    | Pbytes_of_string -> fprintf ppf "bytes_of_string"
    ...
    | Pgetglobal _ | Psetglobal _ -> 
        fprintf ppf "%a" pp_lams args
    | Pmakeblock(_tag, Immutable, _shape) ->
        let lams ppf largs = list ~sep:",@ " lam ppf largs in
        fprintf ppf "@[<2>Seq(%a)@]" lams args
    | Pmakeblock(_tag, Mutable, _shape) -> todo "mutable block"
(*         fprintf ppf "makemutable %i%a" tag block_shape shape *)
    | Pfield n -> fprintf ppf "%a.s_field_access_s(%i)" lam (List.hd args) n
    | Pfield_computed -> fprintf ppf "field_computed"
    | Psetfield(n, ptr, init) ->
        let instr =
          match ptr with
    ...
    | Psequor -> fprintf ppf "||"
    | Pnot -> fprintf ppf "not"
    | Pnegint | Pnegfloat -> pp_binary_prim_op ppf "~" args
    | Paddint | Paddfloat -> pp_binary_prim_op ppf "+" args
    | Psubint | Psubfloat -> pp_binary_prim_op ppf "-" args
    | Pmulint | Pmulfloat -> pp_binary_prim_op ppf "*" args
    | Pdivint Safe -> pp_binary_prim_op ppf "/" args
    (* START TODO *)
    | Pdivint Unsafe -> fprintf ppf "/u"
    | Pmodint Safe -> fprintf ppf "mod"
    | Pmodint Unsafe -> fprintf ppf "mod_unsafe"
    ...
    | Pasrint -> fprintf ppf "asr"
    (* END TODO *)
    | Pintcomp(Ceq) | Pfloatcomp(Ceq) -> pp_binary_prim_op ppf "==" args
    | Pintcomp(Cneq) | Pfloatcomp(Cneq) -> pp_binary_prim_op ppf "!=" args
    | Pintcomp(Clt) | Pfloatcomp(Clt) -> pp_binary_prim_op ppf "<" args
    | Pintcomp(Cle) | Pfloatcomp(Cle) -> pp_binary_prim_op ppf "<=" args
    | Pintcomp(Cgt) | Pfloatcomp(Cgt) -> pp_binary_prim_op ppf ">" args
    | Pintcomp(Cge) | Pfloatcomp(Cge) -> pp_binary_prim_op ppf ">=" args
    (* START TODO *)
    | Poffsetint n -> fprintf ppf "%i+" n
    | Poffsetref n -> fprintf ppf "+:=%i"n
    | Pintoffloat -> fprintf ppf "int_of_float"
  ...
  let _structured_constant = struct_const

end

let strato_lambda fmt lambda = Strato_lambda.lam fmt lambda

let lambda fmt lambda =
  Format.fprintf fmt "/* OCaml lambda:@.%a@.*/@.@.%a\n"
    lam lambda strato_lambda lambda
